...
Framework-specific guidance. Cross-repo conventions (.NET 10 / `LangVersion: preview`, `TreatWarningsAsErrors`, the five analyzers, Central Package Management, code style, PR flow, MTP usage, prose rules) live once in the workspace `../CLAUDE.md`: do not re-derive them here.
...
Source/
  ...
  Core/          MMCA.Common.{Shared,Domain,Application,Infrastructure} (+ AI, the optional
                 language-model boundary: Microsoft.Extensions.AI + Anthropic SDK, no MMCA reference)
  ...
  Hosting/       MMCA.Common.{Aspire,Aspire.Hosting,Gateway,Testing,Testing.Aspire,Testing.E2E,
                 Testing.UI,Testing.Architecture}
...
- **AI package (optional, rubric §16)**: `AddMmcaChatClient(configuration)` binds the `Ai` section and, only when `Ai:Enabled`, registers one `IChatClient` built outermost-first as `BoundedChatClient -> [GuardrailChatClient] -> UsageRecordingChatClient -> [DistributedCache] -> OpenTelemetry -> Logging -> provider`. `GuardrailChatClient` is added **only when the host registered at least one `IChatGuardrail`**; the framework ships the extension point and no content policy (ADR-120), and the streaming path inspects the request only. `BoundedChatClient` clamps output tokens, enforces a per-call timeout, strips tools while `AllowTools` is false, and refuses a request over `PerCallInputTokenBudget` on an ESTIMATE (four chars per token unless an `IAiTokenEstimator` is present). `PromptContract` carries a normalized SHA-256 `Hash` an evaluation gate keys recorded answers on. **The package takes no MMCA.Common project reference and nothing in the framework references it** (`AiDependencyIsolationTestsBase` + `EnforceAiLayerBoundary` hold both directions). When `Ai:Enabled` is false nothing is registered, so consumers gate on `GetService<IChatClient>()`, not on a flag.
