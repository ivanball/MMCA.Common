...
Source/
  ...
  Core/          MMCA.Common.{Shared,Domain,Application,Infrastructure} (+ AI, the optional
                 language-model boundary: Microsoft.Extensions.AI + the Anthropic SDK, no MMCA reference)
...
- **FeatureGate**: short-circuits when the command/query's feature flag is off. Flag constants live on static `*Features` classes and declare their lifecycle with `[FeatureFlag(FeatureFlagLifetime.Permanent|Temporary, RemoveBy, Owner)]` (`MMCA.Common.Shared.FeatureFlags`, ADR-031); `FeatureFlagLifecycleTestsBase` fails the build on an undeclared flag and on a temporary one past its `RemoveBy`, which is the dead-toggle detector. `FeatureFlagRegistry` reports the same inventory at runtime.
- **Authorization**: `IRequiresPermission` commands/queries are checked against `IPermissionRegistry.HasPermission(ICurrentUserService.Roles, ...)`, then `IRequiresMfa` ones against the principal's `mfa` claim; a denial short-circuits with a `Forbidden` error and increments `cqrs.authorization.denied.count`. Both gates live in the shared `AuthorizationGate` so commands and queries cannot drift. Sits outside caching on purpose, so a denied query never reads or populates the cache. Opt-in `AddStoredPermissionGrants(config)` layers a `PermissionGrant` table over the compiled registry through `LayeredPermissionRegistry` (union only, never a stored denial) without changing the decorators' contract.
- **Logging**: full pipeline duration via `ICorrelationContext`. An exception is logged here as a **Warning** outcome line WITHOUT the exception object and rethrown: the boundary that handles it (`GlobalExceptionHandler`/`DbUpdateExceptionHandler`, or `InternalCommandProcessor` for a deferred command) owns the single Error row with the stack, and the shared correlation id joins the two.
...
- **Four engines, one context class each**: `SQLServerDbContext`, `PostgreSQLDbContext`, `SqliteDbContext`, `CosmosDbContext` (ADR-006/018/113). PostgreSQL takes the SQL Server mapping unchanged (table per entity inside the module schema, PascalCase identifiers), so an entity moves between the two by changing its configuration base class alone; it differs only where the server forces it (partial-index predicates use `"Quoted"` identifiers, the soft-delete predicate is `"IsDeleted" = false`, and every `DateTime` is mapped `timestamp with time zone` through `UtcDateTimeConverter`, never the process-wide `Npgsql.EnableLegacyTimestampBehavior` switch).
- **Logical -> physical collapse** (`DataSourceResolver`, singleton): the `DataSources` config section maps logical names to connection strings (plus optional `SQLServerMigrationsAssembly`, `PostgreSQLMigrationsAssembly`, `SqliteMigrationsAssembly`, `CosmosDatabaseName`). Names without an entry, or matching the top-level `ConnectionStrings` value, collapse onto `Default`; a host with no `DataSources` config behaves exactly like a single-database monolith (one context, FK constraints intact). A host selects PostgreSQL with `ConnectionStrings:PostgreSQLConnectionString` (or `DataSources:<Name>:PostgreSQLConnectionString`); a source that names no migrations assembly is created via `EnsureCreated` rather than migrated, the SQLite rule rather than the SQL Server one.
...
- **Design time**: `DesignTimeDbContextHelper.CreateSqlServer(args, ...)` (and the `CreatePostgreSQL` / `CreateSqlite` peers) builds a per-source context for `dotnet ef ... -- --datasource <Name>`, enabling one migrations project per database.

**SaveChanges flow**: stamp audit fields -> capture domain events -> serialize to `OutboxMessage` entries -> `base.SaveChangesAsync()` (data + outbox in one transaction) -> dispatch local domain events in-process -> mark their outbox rows processed. Integration events (`IIntegrationEvent`) are NOT dispatched in-process: their outbox rows stay unprocessed and the `OutboxProcessor` publishes them via `IMessageBus`, so the registered transport (in-process or broker) determines delivery. Inside a Transactional command all post-save dispatch is deferred until after commit. A command scheduled through `IInternalCommandScheduler` rides the same save: its `InternalCommands` row is added to the caller's context, so it commits with the aggregate change or rolls back with it.

### Outbox Pattern

`OutboxMessage` entries persist atomically with aggregate changes, in the same database as the aggregate (every relational source has its own `OutboxMessages` table; a host drains only its own sources). Each row captures the ambient context of the request that raised the event (`TenantId`, `UserId`, `UserRoles`, `CorrelationId`, alongside the trace/span ids), and `OutboxProcessor` restores it onto the cycle's scope before each row is published or dispatched, overwriting it per row so one row's identity never answers for the next; `BrokerMessageBus` then stamps it as the `MMCA-*` headers (`MMCA.Common.Shared.Messaging.MessageHeaders`) and `IntegrationEventConsumer` restores it on the far side before touching the inbox. Capture and restore both run through the shared `AmbientOrigin` helper, the same one the internal-command queue uses, so the two hops cannot drift. `OutboxProcessor` wakes on signal or a smart wait (sleeps until the earliest pending row becomes eligible: `Outbox:ProcessingDelaySeconds`, default 5s), falling back to `Outbox:PollingIntervalSeconds` (default 2s; deployed environments set 300s to cut idle polling without adding latency). Batches of 50, up to 5 retries, at-least-once delivery, OpenTelemetry metrics. The poll query runs inside an `OutboxPoll` activity that `OutboxPollFilterProcessor` (Aspire package) suppresses from telemetry export.

### Internal Commands (job queue)

Durable deferred work on the outbox's machinery. `IInternalCommandScheduler.ScheduleAsync(command, runAt)` writes an `InternalCommandMessage` row (one `InternalCommands` table per relational source, same as the outbox) on the caller's own `IDbContextFactory` context, so scheduling inside an `ITransactional` command is atomic with the aggregate change; outside a transaction the row is saved immediately and the processor is signalled. `InternalCommandProcessor` claims due rows with a lease (`InternalCommands:LeaseSeconds`), then executes each in a fresh DI scope by resolving `ICommandHandler<TCommand, Result>`, so every decorator applies exactly as it would inline. The scheduling user, tenant and correlation id are captured on the row and restored for the execution (`ScopedUserOverride` decorates `ICurrentUserService`), which is what lets an `IRequiresPermission` command run deferred and keeps audit stamps truthful. A `Result.Failure` and a thrown exception both consume an attempt; retries use exponential backoff with jitter and dead-letter after `InternalCommands:MaxAttempts`. `IInternalCommandAdministration` counts, lists, requeues and purges; `InternalCommandCleanupService` sweeps on retention. The table is mapped unconditionally, so `InternalCommands:Enabled` is never a migration, only a choice about whether this host drains the queue. The poll runs inside an `InternalCommandPoll` activity that `OutboxPollFilterProcessor` also suppresses.
...
- **gRPC transport** (`MMCA.Common.Grpc`): `AddGrpcServiceDefaults()` registers server defaults (`GrpcResultExceptionInterceptor` maps `Result` failures to `RpcException`, plus reflection). `AddTypedGrpcClient<TClient>(serviceName)` wires a client to Aspire service discovery over HTTP/2 cleartext (h2c) with a `JwtForwardingClientInterceptor` and the standard Polly pipeline; target services must serve HTTP/2 on their cleartext endpoint (rationale documented in `DependencyInjection.cs`). Consuming modules never touch the generated protobuf types: a hand-written adapter implementing the module's own interface contract wraps the typed client, and that adapter is the module's **Anti-Corruption Layer** (ADR-007). The monolith-to-service move follows the **Strangler Fig** route (ADR-008): stand the service host up beside the monolith, point the typed client and adapter at it, retire the in-process path last.
...
- **Identity completions (all opt-in, ADR-116)**: `AddTwoFactorAuthentication(config)` registers the RFC 6238 second factor (`Authentication:TwoFactor`, Otp.NET in Infrastructure only); the consumer supplies `ITwoFactorStore` over its own `User` and passes the resolved `ITwoFactorAuthenticator` to `AuthenticationServiceBase`, which then answers `Authentication.TwoFactorRequired` or mints the `mfa` claim. `AddEmailConfirmation(config)` adds hashed single-use confirmation tokens (`Authentication:EmailConfirmation`, sign-in gating only when `RequireConfirmedEmail` is set AND the `User` implements `IEmailConfirmableUser`). `UsersAdminControllerBase<TUserDto>` / `RolesAdminControllerBase` are gated on `AdministrationPermissions.ManageUsers` / `ManageRoles`. Nothing changes for a consumer that adopts none of it: every piece is an optional constructor argument or a separate DI call.
...
- **Managed file storage (ADR-045)**: `IFileStorageService` (`AzureBlobFileStorageService`, `NullFileStorageService` fallback) plus the dependency-free upload guards in `Application/Interfaces/Infrastructure/Storage`: `ImageContentSniffer` for avatars, `DocumentContentSniffer` for documents (pdf / pptx / docx / xlsx / zip / txt / md, accepted only when the BYTES and the extension agree on an allowed `DocumentFormats`, with a zip-bomb guard on the Office Open XML package check), `BlobNames.SanitizeFileName` to reduce a user-supplied name to a blob-safe segment, and `FileUploadOptions.Attachment/Inline` to store the RFC 6266 `Content-Disposition` (plus `Cache-Control`) that makes a blob come back as a download. The options-carrying `UploadAsync` overload is a **default interface member** so existing implementations keep compiling; implementations should override it.
...
- **AI package (optional, rubric section 16)**: `MMCA.Common.AI` is the language-model boundary. `AddMmcaChatClient(configuration)` binds the `Ai` section and, only when `Ai:Enabled`, registers one `IChatClient` built outermost-first as `BoundedChatClient -> [GuardrailChatClient] -> UsageRecordingChatClient -> [DistributedCache] -> OpenTelemetry -> Logging -> provider` (Anthropic's official SDK through its own `AsIChatClient` adapter). `GuardrailChatClient` is added **only when the host registered at least one `IChatGuardrail`**, so an app that adopts none keeps the chain it had; the framework ships the extension point and no content policy (ADR-120), a guardrail answers `GuardrailVerdict.Allow` or `GuardrailVerdict.Block(reason)`, a block throws `ChatGuardrailException`, and the streaming path inspects the request only. `UsageRecordingChatClient` also records `mmca.ai.call.duration` with an `outcome` of success/error/canceled beside the two token counters. `BoundedChatClient` clamps output tokens, enforces a per-call timeout on a linked token, strips tools while `AllowTools` is false, and refuses a request over `PerCallInputTokenBudget` on an ESTIMATE (four characters per token unless the pipeline offers an `IAiTokenEstimator`). `PromptContract` carries name/version/model/system prompt and a line-ending-normalized SHA-256 `Hash` that an evaluation gate keys its recorded answers on. The package takes **no MMCA.Common project reference** and nothing in the framework references it; `AiDependencyIsolationTestsBase` plus `EnforceAiLayerBoundary` hold that line in both directions. When `Ai:Enabled` is false nothing is registered, so consumers gate on `GetService<IChatClient>()`, not on a flag.
- **Aspire package**: `AddServiceDefaults()` configures OpenTelemetry, service discovery, Polly resilience (30s attempt / 60s breaker window / 90s total); `MapDefaultEndpoints()` adds `/health` + `/alive`. The tracing pipeline registers `OutboxPollFilterProcessor` and subscribes the `MMCA.Common.AI` trace source (a literal, since Aspire may not reference the AI package; inert without it). The metrics pipeline subscribes the nine `MMCA.Common.*` meters (the ninth being `MMCA.Common.AI`) plus Polly's own `Polly` meter, so `resilience.polly.strategy.events` (retries, timeouts, circuit opens on every HttpClient and gRPC client) is exported; Polly's two duration histograms are dropped by a View unless a host sets `Telemetry:EnablePollyDurationMetrics=true` (rubric §31).
