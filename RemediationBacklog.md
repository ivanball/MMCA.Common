...
Derived from `ArchitectureScorecard.md` (canonical two-axis scoring: **Maturity 95.1% / Implementation 84.3%**, framework v1.115.0; the 2026-07-15 nineteenth-wave two-pass re-score at HEAD `c7b3cb9`, working tree dirty (an ADR-adding pass in flight: ADRs 046-048 untracked), moved §22 Responsive Maturity 3→4 (firefox promoted to a required merge gate 2026-07-12, `ci.yml:112,115`), applied the eighteenth-wave §23 Front-End Performance Maturity 3→4 to the table (the `WebVitalsE2ETests` budget gate in the blocking chromium `ui-e2e` job), recalibrated §12 Performance Maturity 4→3 (the `performance-smoke` job is a runs-clean smoke, not a latency-regression gate, `ci.yml:172,193`), refuted three first-pass lifts (§9 and §25 maturity, §33 implementation, all held at prior), re-confirmed all 28 other categories, and refreshed the ADR range to 001-048).
...
**Scope:** the single-axis item counts below are historical (the wave-by-wave ranking is superseded by the two-axis scorecard, per the note above). Under the live two-axis scorecard there are **no N/A categories** (§27 i18n is now scored after ADR-027) and **27 categories sit at Maturity 4** (#1, 2, 3, 4, 5, 6, 7, 8, 10, 11, 13, 14, 15, 16, 18, 19, 20, 21, 22, 23, 24, 26, 27, 28, 29, 32, 34; **#22 and #23 joined 2026-07-15** on the nineteenth-wave re-score, and **#12 left the set** the same day on the runs-clean recalibration); the open work is the **7 categories still below Maturity 4**. Ranked by two-axis priority = (4 − maturity) × weight: **#31** FinOps (Maturity 2, weight 2, computed priority 4) is the highest weighted gap but a documented accepted cap (see *Deliberate / accepted* below), not scheduled work; then the weight-2 Maturity-3 pack at **priority 2** (#9, #12, #17, #25, #30, #33).
...
### [~] #12 · Performance & Scalability: **REOPENED 2026-07-15 (nineteenth-wave re-score: Maturity 4→3; the eighth-wave impl 7→8 and the smoke gate itself stand)** · priority 2
...
#1 SOLID · #2 Design Patterns · #3 Clean Architecture · **#5 Vertical Slice (maturity 3→4 on the slice-cohesion fitness function)** · #7 Microservices Readiness · #8 Data Architecture · #10 Cross-Cutting Concerns · #14 Testability · #15 Best Practices & Code Quality · **#22 Responsive & Cross-Browser (maturity 3→4 on the firefox required merge gate, nineteenth-wave re-score, 2026-07-15; webkit promoted to a required gate too on 2026-07-16 after 11 consecutive green main runs, closing the last advisory leg and the §22 impl 8→9 lever, candidacy recorded)** · **#23 Front-End Performance (maturity 3→4 on the blocking `WebVitalsE2ETests` budget gate, confirmed nineteenth-wave re-score)** · **#24 Forms, Validation & UX Safety (maturity 3→4 on the CI-gated auth-form tests, fourteenth wave)** · **#27 i18n (maturity 3→4 on the fifteenth-wave completion train, 2026-07-03)** · **#29 Resilience (maturity 3→4 on the build-gated restore drill, tenth wave)**
*(All backed by fitness functions: the regression guard is keeping those tests green. This lists the categories that reached Maturity 4 through tracked remediation; under the live two-axis scorecard the full Maturity-4 set is 27 categories, see the Scope note at the top. **#12 Performance & Scalability left this list on 2026-07-15**: the nineteenth-wave re-score recalibrated its Maturity 4→3 because the `performance-smoke` job is a runs-clean smoke, not a latency-regression gate; see the reopened #12 item above.)*
...
- **#22 Responsive: CLOSED at Maturity 4 / Implementation 8 (nineteenth-wave re-score, 2026-07-15).** firefox was promoted to a required merge gate alongside chromium on 2026-07-12 (`.github/workflows/ci.yml:112`, `continue-on-error` now webkit-only, `:115`), clearing the chromium-only gate that held maturity at 3; on the protect list above. Residual, recorded not scheduled: webkit stays advisory until proven reliably green, and Implementation holds at 8 (the promotion is enforcement, not new responsive execution breadth).
...
- **#25 Navigation, DONE for the shared surface (eighth wave, impl 7→8):** an in-shell `Forbidden` (403) page + `NavigationFlow.md` for the Common UI surface. Per-actor module flows remain consumer-scored. *(Nineteenth-wave note: a proposed maturity 3→4 lift was adversarially refuted and §25 holds at M3/I8. `NavigationFlow.md` has no drift gate and the `Routes.razor` route-auth wiring has no test; only leaf behaviors (`NavMenuTests` role filtering, the `Forbidden` render smoke) run in CI. The path to M4 is a routing fitness check or an `AuthorizeRouteView` wiring test; see the scorecard risk list.)*
