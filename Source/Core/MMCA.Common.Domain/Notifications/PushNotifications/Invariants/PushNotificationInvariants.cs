...
public static class PushNotificationInvariants
{
    ...
    public static readonly int BodyMaxLength = 2000;
    ...
    public static Result EnsureTitleIsValid(string title, string source) =>
        Result.Combine(
            ...
            CommonInvariants.EnsureStringMaxLength(title, TitleMaxLength, "PushNotification.Title.TooLong", string.Create(CultureInfo.InvariantCulture, $"Notification title cannot exceed {TitleMaxLength} characters."), source, nameof(title)));
    ...
    public static Result EnsureBodyIsValid(string body, string source) =>
        Result.Combine(
            ...
            CommonInvariants.EnsureStringMaxLength(body, BodyMaxLength, "PushNotification.Body.TooLong", string.Create(CultureInfo.InvariantCulture, $"Notification body cannot exceed {BodyMaxLength} characters."), source, nameof(body)));
}
