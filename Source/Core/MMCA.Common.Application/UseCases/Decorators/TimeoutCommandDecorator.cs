...
/// <summary>
/// Decorator that enforces a per-command execution budget. Commands that do not implement
/// <see cref="IHasTimeout"/> pass through unchanged. When a command does, the decorator links a
...
/// </para>
/// <para>
/// Registered inside validation and outside the transaction, so the budget covers the database
/// work (the part that actually hangs) without charging the caller for validation, and an expired
/// budget cancels the transaction rather than leaving it open.
/// </para>
/// <para>
...
public sealed class TimeoutCommandDecorator<TCommand, TResult>(
    ICommandHandler<TCommand, TResult> inner) : ICommandHandler<TCommand, TResult>
{
    ...
    {
        if (command is not IHasTimeout hasTimeout || hasTimeout.Timeout <= TimeSpan.Zero)
        ...
        catch (OperationCanceledException) when (budget.IsCancellationRequested && !cancellationToken.IsCancellationRequested)
        {
            ...
            return createFailure([Error.Failure(
                ...
                string.Create(
                    ...
                    $"The command did not complete within its {hasTimeout.Timeout.TotalSeconds:0.###}s budget."),
                source: commandName)]);
        }
    }
}
